...
write_failed_app $res "${app_cmd}"
sleep 1
app_cmd=">>> rhc app create c${pre_app}db $1  -l $user_name -p $passwd --no-git mysql-5.1 mongodb-2.2 postgresql-8.4 rockmongo-1.1 phpmyadmin-3.4 cron-1.4 metrics-0.1"
echo ${app_cmd}
rhc app create c${pre_app}dbcronmms $1  -l $user_name -p $passwd --no-git mysql-5.1 mongodb-2.2 postgresql-8.4 rockmongo-1.1 phpmyadmin-3.4 cron-1.4 metrics-0.1
res=`echo $?`
write_failed_app $res "${app_cmd}"
sleep 1
...
write_failed_app $res "${app_cmd}"
sleep 1
app_cmd=">>> rhc app create c${pre_app}sdb $1  -l $user_name -p $passwd --no-git mysql-5.1 mongodb-2.2 postgresql-8.4 -s"
echo ${app_cmd}
rhc app create c${pre_app}sdb $1  -l $user_name -p $passwd --no-git mysql-5.1 mongodb-2.2 postgresql-8.4 -s
res=`echo $?`
write_failed_app $res "${app_cmd}"
sleep 1
...
end_seconds=`date +%s`
let cost_snd=${end_seconds}-${begin_seconds}
...
echo "======================================================="
echo "============NOTE: Idle app and add MMS cart============"
echo "======================================================="
