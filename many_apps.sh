...
do
select_num=`expr $gearsize_select_num % 3`
if [ ${select_num} -eq 2 ]
then
  gearsize='large'
elif [ ${select_num} -eq 1 ]
then
  gearsize='medium'
else
  gearsize='small'
fi
gearsize_select_num=$(($gearsize_select_num+1))  
...
app_cmd="rhc app create c${pre_app}${suffix_app_name} $1  -l $user_name --no-git ${create_params} -g $gearsize"
echo ${app_cmd}
...
suffix_app_name=`echo $apps | cut -d ' ' -f 2`
create_params=`echo $apps | cut -d ' ' -f 3-`
app_cmd="rhc app create c${pre_app}s${suffix_app_name} $1  -l $user_name --no-git -s ${create_params} -g $gearsize"
echo $app_cmd
eval $app_cmd || echo $app_cmd >> ./failed_apps.txt
sleep ${SLEEP_TIME}
if [[ `echo "${suffix_app_name}" | grep "stopped"` ]]
...
app_cmd="rhc app create c${pre_app}${suffix_app_name} $1  -l $user_name -p $passwd --no-git ${create_params} -g $gearsize"
echo ${app_cmd}
eval $app_cmd || echo $app_cmd >> ./failed_apps.txt
sleep ${SLEEP_TIME}

app_cmd="rhc app create c${pre_app}s${suffix_app_name} $1  -l $user_name -p $passwd --no-git -s ${create_params} -g $gearsize"
echo $app_cmd
eval $app_cmd || echo $app_cmd >> ./failed_apps.txt
sleep ${SLEEP_TIME}
