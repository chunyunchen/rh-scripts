...
class AOS(object):
    ...
    def deploy_logging(cls):
        ...
        if AOS.imageVersion >= "3.3.0":
           ...
           AOS.run_ssh_command("oc create configmap logging-deployer  --from-literal kibana-hostname={}.{} --from-literal public-master-url={} --from-literal es-cluster-size={} --from-literal enable-ops-cluster={} --from-literal use-journal={} --from-literal kibana-ops-hostname={}.{} --from-literal es-instance-ram={} --from-literal es-pvc-dynamic=true --from-literal es-pvc-size=10G --from-literal es-ops-pvc-size=10G".format(AOS.kibanaAppname,subdomain,AOS.MasterURL,AOS.ESClusterSize,AOS.enableKibanaOps,AOS.useJournal,AOS.kibanaOpsAppname,subdomain,AOS.ESRam), ssh=False)
